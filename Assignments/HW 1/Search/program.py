...
def bfs():
    ...
    if checkState(input_node.data):
        ...
        print("No of states expanded = " + str(states_expanded))
    ...
    print("Goal state not reached by depth " + str(max_depth) + ". Failure.\n")
    return
...
def dls(max_d):
    ...
    if checkState(input_node.data):
        print("the input state is the goal state\n")
        print("No of moves = 0")
        print("No of states enqueued (in the last depth limited search) = 0")
        print("No of states expanded (in the last depth limited search) = 0")
        printState(input_node.data)
        return input_node, 0, 0
    ...
    while len(states) > 0:
        ...
        if a > 0:  # up
            ...
            if checkState(new_grid):
                printAllStates(new_node_u)
                print("No of moves = " + str(level + 1))
                print("No of states enqueued (in the last depth limited search) = " + str(count_enqued))
                print("No of states expanded (in the last depth limited search) = " + str(states_expanded))
                return new_node_u, count_enqued, states_expanded
            if (level + 1) < max_d:
        ...
        if a < 2:  # down
            ...
            if checkState(new_grid):
                ...
                print("No of states enqueued (in the last depth limited search) = " + str(count_enqued))
                print("No of states expanded (in the last depth limited search) = " + str(states_expanded))
                return new_node_d, count_enqued, states_expanded
            if (level + 1) < max_d:
        ...
        if b > 0:  # left
            ...
            if checkState(new_grid):
                ...
                print("No of states enqueued (in the last depth limited search) = " + str(count_enqued))
                print("No of states expanded (in the last depth limited search) = " + str(states_expanded))
                return new_node_l, count_enqued, states_expanded
            if (level + 1) < max_d:
        ...
        if b < 2:  # right
            ...
            if checkState(new_grid):
                ...
                print("No of states enqueued (in the last depth limited search) = " + str(count_enqued))
                print("No of states expanded (in the last depth limited search) = " + str(states_expanded))
                return new_node_r, count_enqued, states_expanded
            if (level + 1) < max_d:
                states.append(new_node_r)
                count_enqued += 1
    return None, count_enqued, states_expanded
...
def ids():
    ...
    while d <= max_depth:
        ...
        se += b
        if res is not None:
            print("No of states enqueued (in the entire iterative deepening search) = " + str(sq))
            print("No of states expanded (in the entire iterative deepening search) = " + str(se))
            return
        d += 1
    print("Goal state not reached by depth " + str(max_depth) + ". Failure.\n")
    return
...
def astar(heuristic):
    ...
    pqueue = []
    if heuristic == 0:
        input_node.f = h1(input_node) + input_node.level
    else:
        input_node.f = h2(input_node) + input_node.level
    ...
    while len(pqueue) > 0:
        ...
        if a > 0:  # up
            ...
            new_node_u = Node(new_grid, level + 1, 0, curr_node)
            if heuristic == 0:
                new_node_u.f = h1(new_node_u) + new_node_u.level
            else:
                new_node_u.f = h2(new_node_u) + new_node_u.level
            if checkState(new_grid):
        ...
        if a < 2:  # down
            ...
            new_node_d = Node(new_grid, level + 1, 0, curr_node)
            if heuristic == 0:
                new_node_d.f = h1(new_node_d) + new_node_d.level
            else:
                new_node_d.f = h2(new_node_d) + new_node_d.level
            if checkState(new_grid):
        ...
        if b > 0:  # left
            ...
            new_node_l = Node(new_grid, level + 1, 0, curr_node)
            if heuristic == 0:
                new_node_l.f = h1(new_node_l) + new_node_l.level
            else:
                new_node_l.f = h2(new_node_l) + new_node_l.level
            if checkState(new_grid):
        ...
        if b < 2:  # right
            ...
            new_node_r = Node(new_grid, level + 1, 0, curr_node)
            if heuristic == 0:
                new_node_r.f = h1(new_node_r) + new_node_r.level
            else:
                new_node_r.f = h2(new_node_r) + new_node_r.level
            if checkState(new_grid):
    ...
    if final_node.f < float("inf"):
        ...
        print("No of states expanded = " + str(expanded))
    else:
        print("Goal state not reached by depth " + str(max_depth) + ". Failure.\n")
    return
